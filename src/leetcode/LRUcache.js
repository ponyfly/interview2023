/*
* 解题思路：最近使用的放在最左边，最近没用的放在最右边，考虑到要移动数据，肯定不能使用数组（数组移动复杂度o(N)）,所以解决方案呼之欲出：
* 链表（处理新老关系）+ hashMap(查找在不在)
* 1. 底层使用链表，最近使用的放在左边，最近没用的放在右边，新增的数据放在左边，如果超出容量，则删除最右边节点；再有就是把链表中的一个节点放到尾部（数组复杂度O(N)）
* 2. 查找数据在不在使用hashmap
* 3. 删除操作的时候，我们需要知道前一个节点，修改前一个节点的next，所以使用双向链表
*
* */
